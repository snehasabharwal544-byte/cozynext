...
document.addEventListener("DOMContentLoaded", () => {
  ...
  reviewForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // Reset layout fields to baseline state only upon success
      reviewForm.reset();
      ratingInput.value = "0";
      highlightStars(0);

    } catch (globalError) {
      console.error("Submission error caught safely:", globalError);
      alert("An unexpected error occurred. Please try again.");
    } finally {
      // This resets your button state no matter what happens inside the try block
      submitBtn.innerText = "Submit Review";
      submitBtn.disabled = false;
    }
    // === CORRECTED REGION END ===
